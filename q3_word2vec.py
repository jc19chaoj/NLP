...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x = x/(np.reshape(np.sqrt(np.sum(x**2, axis=1)), (-1, 1))+1e-10)
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    labels[target] = 1
    outputs = softmax(np.dot(predicted, outputVectors.T))
    
    cost = -np.sum(labels * np.log(outputs))
    error_outputs = outputs - labels
    
    gradPred = np.dot(error_outputs, outputVectors)
    grad = np.dot(error_outputs.T, predicted)
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    for i in range(K):
        ...
        neg_activate = sigmoid(-np.dot(predicted.reshape(-1), outputVectors[idx].T))
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for word in contextWords:
        c, g_in, g_out = word2vecCostAndGradient(input_vector, tokens[word], outputVectors, dataset)
        cost+=c
